...
Both keep `played_ids` / `played_titles` / `_RECENT` when they drop tracks — those exclusions are exactly why the refill is new — and both hand the discarded tracks to `_drop_pending_downloads`, which cancels their *queued* downloads with `end_station`'s guards (never a track another station still lists or a `/media` reader is on, and `cancel()` rather than `finish('failed')`, which would impose a retry cooldown the track did nothing to earn). Neither refills synchronously: the station loop's next tick tops the tail back up, where resolving a replacement inline would cost the caller a yt-dlp round trip.
